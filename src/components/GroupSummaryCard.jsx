...
import { Copy, Check, MessageSquare, Image as ImageIcon, MessageCircle } from 'lucide-react';
...
export default function GroupSummaryCard({ settlements, restaurantName, grandTotal }) {
    ...
    const handleCopy = async () => {
        const msg = generateGroupMessage();
        try {
        ...
        >
            <div className="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p className="text-xs font-bold text-text-muted mt-0.5">Copy and paste to your group chat!</p>
                </div>
            </div>
            ...
            <div data-actions className="flex flex-col gap-2.5 mt-2">
                ...
                <motion.button
                    whileTap={{ scale: 0.97 }}
                    onClick={handleCopy}
                    className={`w-full py-2 rounded-full text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-sm ${copied
                            ? 'bg-mint-light border border-mint text-mint-dark'
                            : 'bg-white border-2 border-gray-100 text-text-secondary hover:bg-gray-50'
                        }`}
                >
                    {copied ? <><Check size={14} /> Text Copied!</> : <><Copy size={14} /> Copy as Text</>}
                </motion.button>
            </div>
        </motion.div>
    );
